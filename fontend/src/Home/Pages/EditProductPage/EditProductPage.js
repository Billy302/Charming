...
function EditProduct() {
  ...
  // Quill.js
  // productCopy
  const [productCopy, setProductCopy] = React.useState({ value: null })
  ...
  const fetchProducts = async () => {
    const response = await fetch(
      `http://localhost:3001/Sales/api/product/${paramsID}`
    )
    const data = await response.json()
    let picOrign = data[0].pic_path.split(' ')
    for (let i = 0; i < picOrign.length; i++) {
      let image = document.getElementById(`image${i + 1}`)
      image.src = `http://localhost:3000/Home/ProductImg/${picOrign[i]}`

      if (picOrign.length > 1) {
        for (let i = 0; i < picOrign.length - 1; i++) {
          picPath += picOrign[i] + ' '
        }
        // 最後一筆，不用加空白做區隔
        picPath += picOrign[picOrign.length - 1]
      } else {
        picPath = picOrign[0]
      }
    }
    ...
    setProductType(data[0]['type_id'])
    setProductCopy({ value: data[0]['product_copy'] })
  }
