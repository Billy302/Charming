...
$testForId = $_GET['id']? intval($_GET['id']): 1;

$tagID = $pdo->query("SELECT * FROM `blog_tagtoblog` INNER JOIN `tags` ON `blog_tagtoblog`.`tags_ID` = `tags`.`tags_id` where blog_ID = $testForId;")->fetchAll(); 

$tag_ids = array_column($tagID, 'tags_id');

if(!isset($_GET['id'])){
    header("Location: articleList.php");exit;
    }
    
    $id = intval($_GET['id']);
    $row = $pdo->query("SELECT * FROM `blog` WHERE blog_id=$id")->fetch();
            ...
            <form name="form1" onsubmit="sendData(); return false">
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" name="blog_content" id="blog_content"
                        rows="20"><?= $row['blog_content'] ?></textarea>
                ...
                <div class="mb-3">
                    ...
                    <div>
                        <input type="checkbox" name="tags[]" value="<?= $t['tags_id']?>" id="<?= $t['tags_id']?>"
                            <?= in_array($t['tags_id'], $tag_ids) ? 'checked' : '' ?>>
                ...
                <button type="submit" class="newinfo">修改</button>
            </form>
        </div>
    </div>
</div>
...
// const modal = new bootstrap.Modal(document.getElementById('exampleModalLabel'));
